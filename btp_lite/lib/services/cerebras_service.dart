...
/// Calls Cerebras Cloud (OpenAI-compatible chat completions endpoint)
class CerebrasService {
  ...
  Future<({String text, int? tokens})> complete({
    ...
    if (response.statusCode != 200) {
      ...
      throw Exception('Cerebras API error ${response.statusCode}: $msg');
    }
  ...
  Future<({String text, int? tokens})> chat({
    ...
    if (response.statusCode != 200) {
      final body = jsonDecode(response.body);
      final msg = body['error']?['message'] ?? response.body;
      throw Exception('Cerebras Chat API error ${response.statusCode}: $msg');
    }
